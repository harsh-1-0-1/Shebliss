  ...
  return (
    <section className="w-full py-10 sm:py-16" style={{ backgroundColor: '#F9F8F6' }}>
      <div className="mx-auto px-4 sm:px-6 lg:px-10 xl:px-16">
        ...
        {/* Grid: large card left + 2×2 right on desktop; single column on mobile */}
        <div className="grid grid-cols-2 sm:grid-cols-3 gap-3 sm:gap-4">
          {/* Large feature card — spans 2 rows on sm+ */}
          {first && (
            <div className="col-span-1 sm:row-span-2">
              <Link to={first.href} className="relative overflow-hidden group block h-full" style={{ minHeight: '340px' }}>
                <img
                  src={first.image}
                  alt={first.name}
                ...
                <div className="absolute bottom-0 left-0 p-5">
                  <p
                    ...
                    style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 500, fontSize: 'clamp(1.2rem, 3vw, 1.8rem)', letterSpacing: '0.04em' }}
                  >
                    {first.name}
                  </p>
                  <span className="mt-1 inline-flex text-[9px] font-bold tracking-[0.2em] uppercase text-[#C6A15E] opacity-0 group-hover:opacity-100 transition-opacity">
                    Shop now →
                  </span>
                </div>
              </Link>
