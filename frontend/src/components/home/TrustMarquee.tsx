...
const Separator = () => (
  <span className="mx-6 text-bone/40 text-[10px] select-none" aria-hidden>✦</span>
);
...
export default function TrustMarquee() {
  const strip = TRUST_MARQUEE_MESSAGES.map((msg, i) => (
    <span key={i} className="inline-flex items-center whitespace-nowrap">
      {i > 0 && <Separator />}
      <span className="text-[12px] font-semibold uppercase tracking-[0.18em] text-bone">{msg}</span>
    </span>
  ));
