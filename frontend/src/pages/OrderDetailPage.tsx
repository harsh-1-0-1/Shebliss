...
export default function OrderDetailPage() {
  ...
  if (isError || !order) {
    return (
      <div className="flex flex-col items-center justify-center py-32 gap-4" style={{ backgroundColor: '#F9F8F6' }}>
        ...
        <Link to="/orders" className="text-[11px] font-bold tracking-[0.14em] uppercase text-[#C6A15E] hover:underline">Back to Orders</Link>
      </div>
    );
  }
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 sm:px-6 py-10 sm:py-14 space-y-5" style={{ backgroundColor: '#F9F8F6' }}>
      <Link to="/orders"
        className="inline-flex items-center gap-1.5 text-[11px] font-bold tracking-[0.14em] uppercase text-[#767676] hover:text-[#1A1A1A] transition-colors">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3 pb-5 border-b border-[#EFECE6]">
        <div>
          <p className="text-[9px] font-bold tracking-[0.28em] uppercase text-[#C6A15E] mb-1">Order</p>
          <h1 className="text-2xl sm:text-3xl text-[#1A1A1A]"
            style={{ fontFamily: "'Cormorant Garamond', Georgia, serif", fontWeight: 500 }}>
            #{order.id}
          </h1>
          <p className="text-[12px] text-[#767676] font-body mt-1">
        ...
        <div className="sm:text-right space-y-1.5">
          ...
          <span className="inline-block px-3 py-1 text-[10px] font-bold tracking-wider uppercase font-body"
      ...
      {!isCancelled && (
        <div className="p-5 sm:p-6 border border-[#EFECE6]" style={{ backgroundColor: '#EFECE6' }}>
          <p className="text-[9px] font-bold tracking-[0.22em] uppercase text-[#767676] mb-5">Order Status</p>
          <div className="flex items-start">
            {STATUS_STEPS.map((step, i) => (
              <div key={step} className="flex-1 flex flex-col items-center relative">
                ...
                <div className={`relative z-10 w-6 h-6 flex items-center justify-center text-[10px] font-bold border-2 ${
                  i <= currentStep ? 'bg-[#1A1A1A] border-[#1A1A1A] text-[#F9F8F6]' : 'bg-[#F9F8F6] border-[#EFECE6] text-[#767676]'
                }`}>
                  {i < currentStep ? '✓' : i + 1}
                </div>
                <span className={`text-[9px] mt-2 capitalize text-center font-body font-semibold tracking-wide ${
      ...
      {isCancelled && (
        <div className="p-5 border border-red-200 text-center" style={{ backgroundColor: '#FEF2F2' }}>
          <p className="text-red-600 font-semibold text-[13px] font-body">This order has been cancelled.</p>
        </div>
      )}
      ...
      <div className="border border-[#EFECE6]" style={{ backgroundColor: '#EFECE6' }}>
        <div className="px-5 py-4 border-b border-[#F9F8F6] flex items-center gap-2">
          ...
          <span className="text-[11px] font-bold tracking-[0.16em] uppercase text-[#1A1A1A]">
          ...
          {order.items.map((item) => (
            <div key={item.id} className="flex items-center gap-4 px-5 py-4">
              ...
              <div className="flex-1 min-w-0">
                <p className="text-[14px] font-medium text-[#1A1A1A] font-body">{item.product_name || `Product #${item.product_id}`}</p>
                {item.selected_options && (
                  <p className="text-[11px] text-[#767676] font-body mt-0.5">{formatSelectedOptions(item.selected_options, null)}</p>
                )}
                <p className="text-[11px] text-[#767676] font-body mt-0.5">Qty: {item.quantity} × {format(item.unit_price)}</p>
              </div>
              <p className="text-[14px] font-bold text-[#1A1A1A] shrink-0 font-body">{format(item.quantity * item.unit_price)}</p>
            </div>
          ))}
        </div>
      </div>
      ...
      {order.payment_id && (
        <p className="text-[11px] text-[#767676] font-body px-1">
